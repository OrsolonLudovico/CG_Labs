...
Let's load the dataset: aortic arteries tissue from mice.
The "SingleCellExperiment" class is a Bioconductor container for storing and manipulating single-cell genomics data.
...
#The sce is the single cell experiment object, created by the loading
sce
```
...
The assay contains the primary data, i.e. matrix of sequencing counts, where rows correspond to features (genes) and columns correspond to samples (cells).
One (or more) assays can be accessed as:
...
The counts themselves are loaded as a sparse matrix
NB: You can access the element as a matrix object.
...
Information about the cells (the columns of the object) is stored in the colData slot, a DataFrame object where rows correspond to cells and columns correspond to metadata fields, e.g., batch of origin, treatment condition.
...
Information about the genes (the rows of the object) is stored in the rowData slot, a DataFrame where each row corresponds to a gene and contains annotations like the transcript length or gene symbol.
...
```{r}
metadata(sce)$lab <- "lab_single_cell"
...
Empty droplets often contain RNA from the ambient solution, resulting in non-zero counts.

A useful diagnostic is the barcode rank plot, which shows the (log-)total UMI count for each barcode (cell) on the y-axis and the (log-)rank on the x-axis.
...
The `barcodeRanks` function can be used to rank the barcodes (cells) by number of UMIs and to estimate the knee and inflection point of the distribution.
...
The knee (blue) and inflection (green) points on the curve mark the transition between two components of the total count distribution.
...
What we can say from these plot? THERE ARE FEW EMPTY DROPLETS (GOOD)


# Filtering poor quality cells (Mitochondrial DNA)

The fact that the droplets are not empty does not automatically mean that they should be kept in the analysis.
They may contain damaged or dying cells.

-Here we remove cells with high percentages of mitochondrial reads.

High mitochondrial gene expression may be an indication of damaged or stressed cells.

-The `perCellQCMetrics` function can be used to compute a set of metrics useful to evaluate the quality of the samples, e.g. the sum of counts.

Here, we are computing the proportion of counts mapping to mitochondrial genes ("mt-" pattern in the gene identifiers).

```{r}
library(scuttle)
library(scater)
is.mito <- grep("mt-", rownames(sce)) #Mt means that we're looking for counts that map to mitocondrial genes
per.cell <- perCellQCMetrics(sce, subsets=list(Mito=is.mito))
summary(per.cell$sum)
```

-The `isOutlier` function uses a data driven threshold to define cells of lower quality compared to the rest of the dataset ( determine which values for a metric are outliers based on median-absolute-deviation).
...
Include proportion of counts mapping to mitochondrial genes in the colData
...
We now have a SingleCellExperiment object with only the high-quality cells.
We may also want to filter the genes to focus on those that are more likely to carry information rather than noise.

-In fact, many of the genes that we stored in the object have no UMI in any cell.
...
-Cell-specific biases are normalized using the `computeSumFactors` method, which implements the deconvolution strategy for scaling normalization from pools of cells (Lun, Bach, and Marioni 2016).
...
cl <- quickCluster(sce) 
sce <- computeSumFactors(sce, clusters=cl) # computeSumFactors normalize cell-specific biases
...
The function returns a SingleCellExperiment object with the estimated size factors stored in the colData.
...
You can access the log normalized data
...
plot(librarySizeFactors(sce),sizeFactors(sce),
     xlab="Library size factors", ylab="Deconvolution size factors",
     log="xy", col=as.integer(factor(cl))); abline(a=0,b=1,col="red")
```

# Highly variable genes (HVGs)

High variable genes are the genes in which we are most interest in ... they are carrying biologically relevant information they don't have a constant expression, but they vary between cells.
...
The fitted value of the trend is used as an estimate of the technical component, and we subtract the fitted value from the total variance to obtain the biological component for each gene. (technical is human error while biological i true)
...
Her, we calculate the principal components starting from the data normalized by scran and using only the selected HVGs (using `subset_row` argument) for a better representation of our biological signal.
...
By default `runPCA` will store the PCs in the `PCA` slot (in the single cell experiment object) or it is possible to specify a different slot name, via the `name` argument.
...
This is a reasonable choice, but it may be a good idea to explore the variance explained by each component to decide the number of components to retain.
...
To try and detect the doublets, we can use statistical algorithms, many of which are implemented in the `scDblFinder` package.
We will focus on one technique that uses simulations to identify droplets that are likely to contain more than one cell.
...
Briefly, the strategy can be described in this way: (discussed in previous theory lectures)
...
One of the most popular clustering methods for single-cell RNA-seq is the Louvain algorithm.
...
We need a reference datasets (taken from Tabula Muris data).
In the `colData` of this object, we have the labels...
